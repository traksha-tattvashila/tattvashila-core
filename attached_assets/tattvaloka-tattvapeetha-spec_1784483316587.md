...
**Eligibility for TRK** is earned through **verified authenticity and time — not activity quotas.** No post-count requirement, no daily-streak requirement, no engagement quota. The bar is:
- Email verification
- Mobile verification
- Government ID verification
- Live selfie / human verification
- Completed onboarding
- A fixed minimum waiting period as TMP (a matter of time passing, not days-active or posts-made)

**Verification data handling.** Government ID and selfie are used only to verify authenticity and check for duplicate identity (one person, one identity) — the raw document and photo are not retained after verification. Whatever minimal, encrypted reference is kept for de-duplication is limited to that purpose alone, and this mechanism should be written down explicitly and precisely once implemented, since "we don't store it" and "we can still detect duplicates" both need to be true at once.
